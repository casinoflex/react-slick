...
import getOnDemandLazySlides from "../utils/getOnDemandLazySlides";
import extractObject from "../utils/extractObject";
import initializedState from "../utils/initializedState";
...
import swipeStart from "../utils/swipeStart";
import swipeMove from "../utils/swipeMove";
import swipeEnd from "../utils/swipeEnd";
import getPreClones from "../utils/getPreClones";
import getPostClones from "../utils/getPostClones";
import getTrackLeft from "../utils/getTrackLeft";
import getTrackCSS from "../utils/getTrackCSS";
...
export default class InnerSlider extends React.Component {
  constructor(props) {
    super(props);
    this.list = null;
    this.track = null;
  ...
  onWindowResized = (setTrackStyle) => {
    ...
    this.debouncedResize = debounce(20, () => this.resizeWindow(setTrackStyle));
  ...
  ssrInit = () => {
    if (this.props.variableWidth) return this.variableWidthTrackStyle();

    const childrenCount = React.Children.count(this.props.children);
    const spec = { ...this.props, ...this.state, slideCount: childrenCount };
    const preClonesCount = getPreClones(spec);
    const postClonesCount = getPostClones(spec);
    const slidesCount = preClonesCount + postClonesCount + childrenCount;
    const trackWidth = (100 / this.props.slidesToShow) * slidesCount;
    const slideWidth = 100 / slidesCount;

    let trackLeft =
      (-slideWidth * (preClonesCount + this.state.currentSlide) * trackWidth) /
      100;
